...
from src.application.use_cases.topic.error import TopicError
...
from src.infrastructure.adapters.inbound.http.middlewares.topic_error_handler import (
  topic_error_handler,
)
...
def create_app(container: containers.DeclarativeContainer) -> FastAPI:
  ...
  app.add_exception_handler(TopicError, topic_error_handler)
